...
export const ToggleGroup: React.FC<ToggleGroupProps> = ({
  ...
  const ToggleRefs: ToggleRefsInterface = {};
  ...
  const toComponent = (option: Option) => {
    ...
    return (
      <div
        key={option.value}
        ref={(ref) => {
          ToggleRefs[`Toggle_${option.value}`] = ref;
        }}
      >
        <input
          ...
          checked={checked}
        />
        <ToggleOption htmlFor={optionId} checked={checked}>
          {option.label}
        </ToggleOption>
      </div>
    );
  ...
  return (
    <ToggleOptions id={input.name} direction={field.direction}>
      {ToggleOptions
        ? ToggleOptions.map(toProps).map(toComponent)
        : input.value}
    </ToggleOptions>
  );
};

const ToggleOptions = ({ direction, children, ...props }) => (
  <div
...
const ToggleOption = ({ checked, htmlFor, children, ...props }) => (
  <label
